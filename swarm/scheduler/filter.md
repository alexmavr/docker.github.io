...
When creating a container or building an image, you use a `constraint`,
`whitelist` or `health` filter to select a subset of nodes to consider for
scheduling.  If a node in Swarm cluster has a label with key `containerslots`
...
The node `whitelist` filter directs the scheduler to only consider a specific
list of nodes for running containers on. The nodes are specified as the value to
the `node` key and separated using the `|` character. For example, the following
